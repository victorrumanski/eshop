 ...
 */
public class AlertsPriceChangeKafkaConsumer {

	private static final Logger LOGGER = LoggerFactory.getLogger(AlertsPriceChangeKafkaConsumer.class);

	@Value("${kafka.bootstrap.servers}")
	private String kafkaBootstrapServers;

	private String zookeeperGroupId = "alerts";
	...
	private static class ConsumerLoop {
		...
		public ConsumerLoop(KafkaConsumer<String, String> consumer) {
			...
			consumer.subscribe(Arrays.asList("BASKET_PRICE_CHANGED"));
			...
			while (!stop) {
				...
				for (ConsumerRecord<String, String> record : records) {
					...
					try {
						...
						List<?> userids = (List<?>) obj.get("userids");
						for (Object uid : userids) {
							LOGGER.info("Sending Email alert of price change for user: " + uid);
						}
					} catch (Exception e) {
						e.printStackTrace();
					}
