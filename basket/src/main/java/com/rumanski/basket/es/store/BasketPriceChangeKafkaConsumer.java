...
import com.rumanski.basket.SpringContext;
import com.rumanski.basket.es.BasketEventHandler;

@Component
/*
 * This class will only log messages from kafka
 */
public class BasketPriceChangeKafkaConsumer {

	private static final Logger LOGGER = LoggerFactory.getLogger(BasketPriceChangeKafkaConsumer.class);

	@Value("${kafka.bootstrap.servers}")
	private String kafkaBootstrapServers;

	private String zookeeperGroupId = "basket-PRICE_CHANGED";
	...
	private static class ConsumerLoop {
		...
		public ConsumerLoop(KafkaConsumer<String, String> consumer) {
			...
			consumer.subscribe(Arrays.asList("PRICE_CHANGED"));
		...
		public void loop() {
			...
			while (!stop) {
				...
				for (ConsumerRecord<String, String> record : records) {
					...
					// AQUI teria que ir item a item nos basket atualizando os preços //
					BasketEventHandler handler = SpringContext.getBean(BasketEventHandler.class);

					try {
						handler.handlePriceChanged(message);
					} catch (Exception e) {
						e.printStackTrace();
						try {
							Thread.sleep(5000);
