...
import com.rumanski.ordering.es.OrderingEventHandler;
...
public class OrderPlacedKafkaConsumer {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(OrderPlacedKafkaConsumer.class);
	...
	private static class ConsumerLoop {
		...
		public void loop() {
			...
			// from basket producer
			List<String> topics = Arrays.asList("ORDER_PLACED");
			...
			while (!stop) {
				...
				for (ConsumerRecord<String, String> record : records) {
					...
					OrderingEventHandler handler = SpringContext.getBean(OrderingEventHandler.class);

					try {
						handler.handleOrderPlaced(message);
					} catch (Exception e) {
						e.printStackTrace();
						try {
							Thread.sleep(5000);
